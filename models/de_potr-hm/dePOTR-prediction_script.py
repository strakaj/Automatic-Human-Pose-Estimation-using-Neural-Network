...
from coco_dataset.coco_dataset import COCODataset, get_final_preds, flip_back
...
dataset_test = COCODataset(args.data_path, args.images_path,
                           transforms.Compose(trans),
                           mode='test',
                           relative_positions=args.relative_positions,
                           crop_w=args.crop_w,
                           crop_h=args.crop_h,
                           heatmap_size=[args.heatmap_size_x, args.heatmap_size_y],
                           sigma=args.sigma)
...
for i, (samples, meta_data) in enumerate(data_loader):
    ...
    if args.flip_test:
        ...
        output_flipped = flip_back(output_flipped,
                                   dataset_test.flip_pairs)

        # feature is not aligned, shift flipped heatmap for higher accuracy
        if shift_heatmap:
            output_flipped[:, :, :, 1:] = \
                copy.deepcopy(output_flipped)[:, :, :, 0:-1]

        results = (results + output_flipped) * 0.5

    # ---------------------- predictions in coco format -------------------------- #
    preds, maxvals = get_final_preds(results,
