...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for item_index, (samples, targets, vis) in enumerate(data_loader):
        samples = [item.to(device, dtype=torch.float32) for item in samples]
        targets = [item.to(device) for item in targets]
        vis = [item.to(device) for item in vis]

        outputs = model(samples)

        for i in range(len(vis)):
            outputs[i] = outputs[i] * vis[i]
...
def evaluate(model, criterion, data_loader, device, print_freq=10):
    ...
    for samples, targets, vis in metric_logger.log_every(data_loader, print_freq, header):
        samples = [item.to(device, dtype=torch.float32) for item in samples]
        targets = [item.to(device) for item in targets]
        vis = [item.to(device) for item in vis]
...
def save_log_imgs(model, criterion, data_loader, device, path):
    ...
    for i, (samples, targets, vis) in enumerate(data_loader):
        if not (i in ids):
            continue
        samples = [item.to(device, dtype=torch.float32) for item in samples]
        targets = [item.to(device) for item in targets]
...
def save_training_summary(model, data_loader, device, log_path, input_log_path, path):
    ...
    model.eval()
    for idx, (samples, targets, vis) in enumerate(data_loader):
        if not (idx in ids):
            continue
        samples = [item.to(device, dtype=torch.float32) for item in samples]
        targets = [item.to(device) for item in targets]
        ...
        for j, joint in enumerate(targets[0]):
            if data_loader.dataset.relative_positions:
