...
from data_utils.coco_info import dataset_info
from data_utils.coco_utils import image_idx_to_ann_idx, only_with_annotations, get_random_idx
...
def visualize_image_annotations(images, annotations, dataset_path, joints=True, connections=True, joints_name=False,
                                alpha=0.5, joints_r=5, connectins_w=5, figsize=(15, 10), path=""):
    max_cols = 2
    images_num = 1
    ...
    nrows = int(np.ceil(images_num / max_cols))
    ncols = max_cols
    if nrows == 1:
...
def visualize_image_annotations2(image, annotations, joints=True, connections=True, joints_name=False,
                                alpha=0.5, joints_r=5, connectins_w=5, figsize=(15, 10), path="", colors=[]):

    nrows = 1
    ncols = 1
    text_overlay = []

    fig, ax = plt.subplots(nrows=nrows, ncols=ncols, frameon=False, figsize=figsize)
    ax.imshow(image)

    ax.set_axis_off()


    for j, anns in enumerate(annotations):
        ...
        for i,k in enumerate(keypoint_connections):
            ...
            kp1_x = kp[kp_id1][0]
            kp1_y = kp[kp_id1][1]
            kp1_v = kp1_x > 0 and kp1_y > 0

            kp2_x = kp[kp_id2][0]
            kp2_y = kp[kp_id2][1]
            kp2_v = kp2_x > 0 and kp2_y > 0
        ...
        for i in range(int(kp.shape[0])):
            ...
            if(v > 0):
                ...
                if colors:
                    color = colors[j]
                if joints:
                    ax.add_patch(Circle(center, radius=joints_r, fc=np.append(color, alpha), ec=np.append(color, 1), lw=1, fill=True))
                if joints_name:
                    text_overlay.append(
                        ax.text(x=x, y=y, s=kp_name, color=color, fontsize=12))
