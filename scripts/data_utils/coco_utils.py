...
def get_stats(annotations):
    ...
    non_labeled = np.sum(list(keypounts_not_labeled.values()))
    ...
    print()
    print('Visible joints: {} ({:.2f}%)'.format(visible, visible/(visible+non_visible+non_labeled) * 100))
    print('Nonvisible joints: {} ({:.2f}%)'.format(non_visible, non_visible/(visible+non_visible+non_labeled) * 100))
    print('Nonlabeled joints: {} ({:.2f}%)'.format(non_labeled, non_labeled/(visible+non_visible+non_labeled) * 100))
    
    return keypoints, keypounts_visible, keypounts_not_visible, keypounts_not_labeled, nannotations
